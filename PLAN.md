...
| 4. Playwright — completed | Build the minimum structure, automate at least five priority cases, configure request interception, two browsers, tags and reports. | [Playwright execution record](docs/playwright-results.md), `playwright.config.ts`, `tests/`; 18 passed, including valid submit and name boundaries; no skipped or expected-failing checks. |
...
- Switch between staging/production through `baseUrl`; use an environment with placeholder values and no secrets. Add the exact Newman command to the README.
...
- [x] A Postman collection and environment exist, and the Newman command was verified in safe blocked mode; the status of actual API-test execution is documented.
...
- [x] The intentional expected failure for BUG-03 remains visible and explained.
